...
export const listAllDrivers = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabaseAdmin
      .from("drivers")
      .select(
        "id, user_id, full_name, email, phone_number, car_number, vehicle_make, vehicle_model, vehicle_type, driving_license, personal_details_json, is_approved, status, rating, total_rides, created_at",
      )
